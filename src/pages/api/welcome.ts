...
function buildWelcomeHtml(args: { name: string; lang: 'es' | 'en' }) {
...
export async function POST({ request }: { request: Request }) {
  try {
    ...
    if (!email || !name) {
      return new Response(JSON.stringify({ success: false, message: 'Requeridos' }), {
        status: 400,
    ...
    if (!user || !pass) {
      return new Response(JSON.stringify({ success: false, message: 'Servidor sin credenciales SMTP' }), {
        status: 500,
        headers: { 'content-type': 'application/json' },
      });
    }
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: lang === 'es' ? 'Registro confirmado: Bienvenido a Slowork' : 'Registration confirmed: Welcome to Slowork',
      html: buildWelcomeHtml({ name, lang }),
    });

    return new Response(JSON.stringify({ success: true, message: 'Enviado' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  } catch (err) {
